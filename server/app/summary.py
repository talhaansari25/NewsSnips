...
# using both cosine similarity and word frequnecy 
...
# 🔥 Final function using both word frequency + cosine similarity with title
def getSummary(text, title=None):
    doc = nlp(text)
    word_frequencies = {}
    getWordFrequenciesFromDoc(word_frequencies, doc)
    ...
    # ➕ Boosting using cosine similarity (optional)
    if title:
        title_doc = ' '.join([token.text for token in nlp(title) if token.text.lower() not in stopwords and token.text not in punctuation])
        for sent in sentence_tokens:
            sent_doc = ' '.join([token.text for token in nlp(sent) if token.text.lower() not in stopwords and token.text not in punctuation])
            sim = cosine_similarity(sent_doc, title_doc)
            sentence_scores[sent] = sentence_scores.get(sent, 0) + sim

    return processSummary(sentence_tokens, sentence_scores, 0.1)
